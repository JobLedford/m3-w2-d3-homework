...
const setTiles = () => {
    for (tiles of tiles) {
      tiles.innerHTML = randomOrderArray[i];
      i++;
      //replace numerical values with icon pairs

      if (tiles.innerText < 3) {
        tiles.innerHTML = rocket;
        tiles.setAttribute("icon", "rocket");
      } else if (tiles.innerHTML < 5) {
        tiles.innerHTML = bacteria;
        tiles.setAttribute("icon", "bacteria");
      } else if (tiles.innerHTML < 7) {
        tiles.innerHTML = cocktail;
        tiles.setAttribute("icon", "cocktail");
      } else if (tile.innerHTML < 9) {
        tiles.innerHTML = football;
        tiles.setAttribute("icon", "football");
      } else if (tile.innerHTML < 11) {
        tiles.innerHTML = pizza;
        tiles.setAttribute("icon", "pizza");
      } else if (tile.innerHTML < 13) {
        tiles.innerHTML = kiwi;
        tiles.setAttribute("icon", "kiwi");
      } else {
...
const resetTiles = () => {
    for (tiles of tiles) {
      tiles.style.backgroundColor = "#44445a";
      tiles.removeAttribute("state");
      tiles.classList.remove("hideTile");
      tiles.classList.remove("displayTile");
    }
}